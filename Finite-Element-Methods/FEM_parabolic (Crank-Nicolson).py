...
def stiff(node,connectivity):
    ...
    A = np.zeros((N,N));
    M = np.zeros((N,N));
    # Call another function, in order to calculate the local stiffness matrix
    for k in range(0,NT):
...
U[:,0] = 0;
for n in range(0,Nt):
    U[:,n] = np.matmul(np.linalg.inv(M+(tau/2)*A),((tau/2)*F+np.matmul(M-(tau/2)*A,U[:,n-1])));
...
U2[:,0] = 0;
for n in range(1,Nt):
    firstterm = np.linalg.inv((tau/2)*A2+M2);
    secondterm = (tau/2)*s+np.matmul(M2-(tau/2)*A2,U2[:,n-1]);
